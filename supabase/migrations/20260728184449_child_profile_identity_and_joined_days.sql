...
-- Support databases that already received the identity-aware function under
-- the previous migration filename as well as databases with the legacy signature.
drop function if exists public.provision_child_account(uuid, text, text, uuid, text, text);
...
drop function if exists public.create_child_profile(uuid, text, text, text, text);
