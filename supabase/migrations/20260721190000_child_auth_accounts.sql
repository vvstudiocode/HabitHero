-- Replace anonymous child password binding with real Supabase Auth users.
-- Existing child data remains intact; existing children need a new account
-- created by the parent because the legacy password hashes are not migrated.
...
-- Legacy anonymous-password functions are no longer part of the application
-- contract. Keep their data for rollback/audit, but prevent new client calls.
