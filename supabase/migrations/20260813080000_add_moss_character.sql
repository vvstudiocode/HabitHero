-- Add the supplied Moss character as a compact animated shop item.
-- If an older deployment already has a child/profile equipped with this key,
-- move that child to a random active character before restoring the immutable
-- identity guard.
...
insert into public.game_catalog_items (
  ...
  'character', '莫斯', '帶著鹿角與森林氣息，安靜地踏上冒險旅程。', 9,
  'character.moss', '/assets/characters/moss-thumbnail.webp',
  true, false, false, 0.28, 0.9, 1.1, 14,
  jsonb_build_object(
    'source', 'User-provided 莫斯.fbx + 莫斯Idle.fbx',
...
do $$
declare
  moss_key constant text := 'character.moss';
  child_row record;
...
begin
  for child_row in
    ...
    where child.character_id = moss_key
       or exists (
         select 1
...
-- New children can select any shipped world character, including Moss. Arthur
-- remains the fallback when a profile has no matching supplied character.
...
declare
  ...
  supplied_character_keys constant text[] := array[
    'character.arthur', 'character.elina', 'character.sia', 'character.elio', 'character.moss'
  ];
begin
