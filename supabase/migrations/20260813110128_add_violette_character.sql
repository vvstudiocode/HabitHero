-- Add the supplied Violette character as a compact animated shop item.
...
insert into public.game_catalog_items (
  ...
  'character', '薇歐莉特', '帶著薰衣草色長髮，在溫柔晨光中展開每日冒險。', 9,
  'character.violette', '/assets/characters/violette-thumbnail.webp',
  true, false, false, 0.28, 0.9, 1.1, 17,
  jsonb_build_object(
    'source', 'User-provided 薇歐莉特.fbx + 薇歐莉特Idle.fbx',
...
do $$
declare
  violette_key constant text := 'character.violette';
  child_row record;
...
begin
  for child_row in
    ...
    where child.character_id = violette_key
       or exists (
         select 1
         ...
         where loadout.child_profile_id = child.id
           ...
           and item.asset_key = violette_key
       )
    order by child.id
  loop
