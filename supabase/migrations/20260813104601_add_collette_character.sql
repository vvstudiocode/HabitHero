-- Add the supplied Collette character as a compact animated shop item.
...
insert into public.game_catalog_items (
  ...
  'character', '柯蕾特', '戴著藍色貝雷帽，穿著紅裙踏上溫暖的小旅程。', 9,
  'character.collette', '/assets/characters/collette-thumbnail.webp',
  true, false, false, 0.28, 0.9, 1.1, 16,
  jsonb_build_object(
    'source', 'User-provided 柯蕾特去背.fbx + 柯蕾特Idle.fbx',
...
do $$
declare
  collette_key constant text := 'character.collette';
  child_row record;
...
begin
  for child_row in
    ...
    where child.character_id = collette_key
       or exists (
         select 1
         ...
         where loadout.child_profile_id = child.id
           ...
           and item.asset_key = collette_key
       )
    order by child.id
  loop
...
declare
  ...
  supplied_character_keys constant text[] := array[
    ...
    'character.moss', 'character.noah', 'character.collette'
  ];
begin
