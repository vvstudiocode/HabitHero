"""Pack the supplied Collette FBX files into one compact animated GLB.

The Idle FBX supplies the shared mesh, materials, and textures. Each other
FBX supplies one action. Walk is copied without normalizing or removing its
authored root motion; the runtime decides whether to consume that motion.
...
OUTPUT = os.path.join(ROOT, "public/assets/characters/collette.glb")
IDLE_SOURCE = "/Users/studio.vv/Desktop/habithero動作檔/柯蕾特Idle.fbx"
...
def main() -> None:
    reset_scene()
    base_armature, base_mesh, idle_source_action = import_fbx(IDLE_SOURCE)
    idle = idle_source_action.copy()
    idle.name = "Idle"
    ...
    for source_path, action_name in (
        ("/Users/studio.vv/Downloads/柯蕾特.fbx", "Walk_InPlace"),
        ("/Users/studio.vv/Downloads/柯蕾特坐下.fbx", "Sit"),
        ("/Users/studio.vv/Downloads/柯蕾特揮手.fbx", "Wave"),
        ("/Users/studio.vv/Downloads/柯蕾特跳舞.fbx", "Dance"),
    ):
    ...
    print("COLLETTE_OUTPUT_BYTES", os.path.getsize(OUTPUT))
