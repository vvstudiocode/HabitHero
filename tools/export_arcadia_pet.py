"""Build the supplied Arcadia FBX pair into one mobile-ready animated GLB.
...
Run with Blender 5.x:
  /Applications/Blender.app/Contents/MacOS/Blender --background --python tools/export_arcadia_pet.py
"""
...
WALK_SOURCE = "/Users/studio.vv/Downloads/阿卡迪亞.fbx"
IDLE_SOURCE = "/Users/studio.vv/Downloads/阿卡迪亞Idle.fbx"
MODEL_OUTPUT = os.path.join(ROOT, "public/assets/pets/arcadia.glb")
THUMBNAIL_SOURCE = "/Users/studio.vv/Downloads/阿卡迪亞去背.png"
THUMBNAIL_OUTPUT = os.path.join(ROOT, "public/assets/pets/arcadia-thumbnail.webp")
...
HORIZONTAL_ROOT_MOTION_AXES = (0, 2)
...
def reduce_mesh(mesh) -> tuple[int, int]:
    ...
    modifier = mesh.modifiers.new(name="ArcadiaMobileDecimate", type="DECIMATE")
...
def export_model() -> None:
    ...
    if walk_action is None:
        raise RuntimeError("阿卡迪亞 walk FBX did not contain an animation action")
    ...
    if idle_action is None:
        raise RuntimeError("阿卡迪亞 Idle FBX did not contain an animation action")
    ...
    print(
        f"ARCADIA_MODEL {MODEL_OUTPUT} original_polygons={original_polygons} "
...
def export_thumbnail() -> None:
    ...
    print(f"ARCADIA_THUMBNAIL {THUMBNAIL_OUTPUT} size={image.size[:]} format={image.file_format}")
