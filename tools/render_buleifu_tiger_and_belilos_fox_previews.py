"""Render quick visual checks for the two shipped pet GLBs."""
...
PETS = (
    ("buleifu-tiger", "/Users/studio.vv/Desktop/HabitHero/public/assets/pets/buleifu-tiger.glb", "/tmp/buleifu-tiger-preview.png", 12),
    ("belilos-fox", "/Users/studio.vv/Desktop/HabitHero/public/assets/pets/belilos-fox.glb", "/tmp/belilos-fox-preview.png", 20),
)


def render(name: str, glb: str, output: str, frame: int) -> None:
    ...
    ground.data.materials.append(bpy.data.materials.new(f"{name}-ground"))
    ...
    for light_name, location, energy, size in (
        ...
        light_data = bpy.data.lights.new(f"{name}-{light_name}", "AREA")
        light_data.energy = energy
        light_data.shape = "DISK"
        light_data.size = size
        light = bpy.data.objects.new(f"{name}-{light_name}", light_data)
    ...
    print(f"{name.upper().replace('-', '_')}_PREVIEW {output}")
