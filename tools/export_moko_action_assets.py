"""Pack the supplied Moko FBX files into one compact animated GLB.
...
``莫可Idle.fbx`` is the only source used for the skinned mesh, materials and
texture. The other files contribute animation clips only, so the exported
asset does not duplicate geometry or textures for each action.
"""
...
OUTPUT = os.path.join(ROOT, "public/assets/pets/moko.glb")
TEXTURE_MAX_SIZE = 1024
TEXTURE_QUALITY = 82
MESH_DECIMATE_RATIO = 0.20
...
def normalize_walk_root_motion(action) -> None:
    ...
    for fcurve in action_channel_fcurves(action):
        # Blender FBX import uses X/Y as the horizontal plane. glTF Y-up maps
        # Blender Y to glTF Z, so both horizontal channels must be in-place.
...
def reduce_mesh(mesh) -> None:
    modifier = mesh.modifiers.new(name="MOKO_MobileDecimate", type="DECIMATE")
...
def main() -> None:
    reset_scene()
    base_armature, base_mesh, idle_source = import_fbx(os.path.join(DOWNLOADS, "莫可Idle.fbx"))
    ...
    for source_path, action_name in (
        (os.path.join(ACTION_DIRECTORY, "莫可.fbx"), "Walk_InPlace"),
        (os.path.join(DOWNLOADS, "莫可坐下.fbx"), "Sit"),
        (os.path.join(DOWNLOADS, "莫可揮手.fbx"), "Wave"),
        (os.path.join(ACTION_DIRECTORY, "莫可跳舞.fbx"), "Dance"),
    ):
    ...
    export_glb()
    print("MOKO_ACTIONS", [action.name for action in actions])
    print("MOKO_OUTPUT_BYTES", os.path.getsize(OUTPUT))
