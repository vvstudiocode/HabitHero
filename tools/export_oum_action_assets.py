"""Pack the supplied Oum FBX files into one compact animated GLB.
...
OUTPUT = os.path.join(ROOT, "public/assets/pets/oum.glb")
...
def reduce_mesh(mesh) -> None:
    ...
    modifier = mesh.modifiers.new(name="OUM_MobileDecimate", type="DECIMATE")
    ...
    print("OUM_MESH_POLYGONS", original_polygons, len(mesh.data.polygons))
...
def main() -> None:
    ...
    base_armature, base_mesh, idle_source = import_fbx(
        os.path.join(ACTION_DIRECTORY, "歐姆 Idle.fbx")
    )
    ...
    for source_path, action_name in (
        (os.path.join(ACTION_DIRECTORY, "歐姆.fbx"), "Walk_InPlace"),
        (os.path.join(ACTION_DIRECTORY, "歐姆坐下.fbx"), "Sit"),
        (os.path.join(ACTION_DIRECTORY, "歐姆揮手.fbx"), "Wave"),
        (os.path.join(ACTION_DIRECTORY, "歐姆跳舞.fbx"), "Dance"),
    ):
    ...
    export_glb()
    print("OUM_ACTIONS", [action.name for action in actions])
    print("OUM_OUTPUT_BYTES", os.path.getsize(OUTPUT))
