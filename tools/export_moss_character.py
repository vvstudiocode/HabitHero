"""Build the supplied Moss FBX pair into one mobile-ready animated GLB.
...
Run with Blender 5.x:
  /Applications/Blender.app/Contents/MacOS/Blender --background --python tools/export_moss_character.py
"""
...
WALK_SOURCE = "/Users/studio.vv/Downloads/莫斯.fbx"
IDLE_SOURCE = "/Users/studio.vv/Downloads/莫斯Idle.fbx"
MODEL_OUTPUT = os.path.join(ROOT, "public/assets/characters/moss.glb")
THUMBNAIL_SOURCE = "/Users/studio.vv/Downloads/莫斯去背.png"
THUMBNAIL_OUTPUT = os.path.join(ROOT, "public/assets/characters/moss-thumbnail.webp")
...
def export_model() -> None:
    ...
    if walk_action is None:
        raise RuntimeError("莫斯 walk FBX did not contain an animation action")
    ...
    if idle_action is None:
        raise RuntimeError("莫斯 idle FBX did not contain an animation action")
    idle_action.name = "Idle"
    lock_root_motion(idle_action)

    # Keep only the first FBX's mesh and rig. The imported idle action keeps
    # its matching bone paths and is still exported as a second action below.
    duplicate_objects = [
        object_
    ...
    print(
        f"MOSS_MODEL {MODEL_OUTPUT} original_polygons={original_polygons} "
...
def export_thumbnail() -> None:
    ...
    print(f"MOSS_THUMBNAIL {THUMBNAIL_OUTPUT} size={image.size[:]} format={image.file_format}")
