...
const unityEditorPath = process.env.HABITHERO_UNITY_EDITOR_PATH
  ...
  || defaultUnityEditorPath;
const resultDirectory = mkdtempSync(path.join(tmpdir(), 'habithero-unity-playmode-'));
const resultPath = path.join(resultDirectory, 'test-results.xml');

if (!existsSync(unityEditorPath)) {
  console.error(
    `找不到 Unity Editor：${unityEditorPath}\n` +
      '可用 HABITHERO_UNITY_EDITOR_PATH 指定其他 Unity 6 Editor 路徑。',
  );
...
try {
  const result = spawnSync(
    ...
    [
      ...
      'playmode',
      '-testFilter',
      'HabitHero.Tests.BootstrapPlayModeTests',
      '-testResults',
      resultPath,
      '-logFile',
      '-',
