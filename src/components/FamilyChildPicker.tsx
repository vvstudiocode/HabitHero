    ...
    <div className="hh-modal-shell hh-switch-modal-shell fixed inset-0 z-[300] flex items-center justify-center bg-black/60 backdrop-blur-xs p-5" role="dialog" aria-modal="true" aria-labelledby="family-child-picker-title">
      <div className="hh-modal-panel hh-switch-modal-panel hh-family-picker-panel w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-slide-up">
        ...
        <div className="mt-5 space-y-2">
          {children.map((child) => (
            <button key={child.id} type="button" onClick={() => onSelect(child.id)} className="hh-family-picker-child-option flex min-h-12 w-full items-center gap-3 rounded-2xl px-4 text-left font-black transition-colors">
              <User size={20} /> {child.name}的任務
            </button>
          ))}
          <button type="button" onClick={onParentMode} className="hh-family-picker-parent-option flex min-h-12 w-full items-center gap-3 rounded-2xl px-4 text-left font-black transition-colors">
