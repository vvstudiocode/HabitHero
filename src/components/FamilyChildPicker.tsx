...
export function FamilyChildPicker({ children, onSelect, onParentMode }: FamilyChildPickerProps) {
  return (
    <div className="hh-modal-shell hh-switch-modal-shell fixed inset-0 z-[300] flex items-center justify-center bg-black/60 backdrop-blur-xs p-5" role="dialog" aria-modal="true" aria-labelledby="family-child-picker-title">
      <div className="hh-modal-panel hh-switch-modal-panel w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl animate-slide-up">
        ...
        <div className="mt-5 space-y-2">
          {children.map((child) => (
            <button key={child.id} type="button" onClick={() => onSelect(child.id)} className="flex min-h-12 w-full items-center gap-3 rounded-2xl border border-yellow-100 bg-yellow-50 px-4 text-left font-black text-yellow-950 hover:bg-yellow-100 transition-colors">
              <User size={20} /> {child.name}的任務
            </button>
          ))}
          <button type="button" onClick={onParentMode} className="flex min-h-12 w-full items-center gap-3 rounded-2xl border border-blue-100 bg-blue-50 px-4 text-left font-black text-blue-900 hover:bg-blue-100 transition-colors">
