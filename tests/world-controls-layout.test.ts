...
test('landscape world controls place the action cluster to the right of chat', () => {
  ...
  assert.match(overlays, /@media \(orientation:\s*landscape\)[\s\S]*?\.hh-friend-dock\s*\{[\s\S]*?left:\s*calc\(50% \+ var\(--hh-world-chat-landscape-half-width\) \+ \(var\(--hh-world-control-size\) \+ var\(--hh-world-control-gap\)\) \* 2 \+ var\(--hh-world-control-gap\)\);/);
